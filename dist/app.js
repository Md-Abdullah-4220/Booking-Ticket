...
// const CreatPriceSeat = document.querySelectorAll("#creatPriceSeat");
let calclution
...
searchBtn.forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    if (count < 4 && btn.classList.contains("bg-gray-100")) {
      ...
      newElement.className = "flex justify-between text-center mt-5 text-[#03071299]";
      newElement.setAttribute("id", inText); // Add a unique identifier
      ...
      const elementToRemove = document.getElementById(inText);
      if (elementToRemove) {
...
document.getElementById("actionCoupon").addEventListener("click", function () {
  if (EnterCoupon.value == "NEW15") {
    ...
    GrandTotal.innerHTML = calclution;
    EnterCoupon.disabled = true;
  }
});
